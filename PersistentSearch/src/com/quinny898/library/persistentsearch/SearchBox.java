...
public class SearchBox extends RelativeLayout {
	...
	public SearchBox(Context context, AttributeSet attrs, int defStyle) {
		...
		search.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				if (s.length() > 0) {
					...
					mic.setImageDrawable(getContext().getResources()
		...
		mSearchFilter = new SearchFilter() {
			...
			public boolean onFilter(SearchResult searchResult, String searchTerm) {
				return searchResult.title.toLowerCase().startsWith(
	...
	/***
	 * Hide the searchbox using the circle animation which centres upon the
	 * provided menu item. Can be called regardless of result list length
	 * 
	 * @param id
	 *            ID of menu item
	 * @param activity
	...
	public void hideCircularlyToMenuItem(int id, Activity activity) {
		...
		if (menuButton != null) {
			...
			if (layout.findViewWithTag("searchBox") == null) {
				int[] location = new int[2];
				menuButton.getLocationInWindow(location);
				hideCircularly(location[0] + menuButton.getWidth() * 2 / 3,
						location[1], activity);
			}
		}
	}
	...
	public void hideCircularly(Activity activity) {
		final FrameLayout layout = (FrameLayout) activity.getWindow()
				.getDecorView().findViewById(android.R.id.content);
		hideCircularly(layout.getLeft() + layout.getRight(), layout.getTop(),
				activity);
	}
	...
	/***
	 * Populate the searchbox with words, in an arraylist. Used by the voice
	 * input
	 * 
	 * @param matches
	 *            Matches
	...
	/***
	 * 
	 * Set the results that are shown (up to 5) when the searchbox is opened
	 * with no text
	 * 
	 * @param results
	 *            Results
	...
	/***
	 * Set whether to search without suggestions being available (default is
	 * true). Disable if your app only works with provided options
	 * 
	 * @param state
	 *            Whether to show
	 */
	...
	/***
	 * Set the image drawable of the drawer icon logo (do not set if you have
	 * not hidden the menu icon)
	 * 
	 * @param icon
	 *            Icon
	 */
	...
	/***
	 * Set the SearchFilter used to filter out results based on the current
	 * search term
	 * 
	 * @param filter
	 *            SearchFilter
	...
	private void search(SearchResult result) {
		if (!searchWithoutSuggestions && getNumberOfResults() == 0)
			return;
	...
	/***
	 * Set to false to retain the logo from setDrawerLogo() instead of animating
	 * to the arrow during searches.
	 * 
	 * @param show
	 *            Should the SearchBox animate the drawer logo
	 */
	...
	public interface SearchFilter {
		/**
		 * Called against each Searchable to determine if it should be filtered
		 * out of the results
		 */
		public boolean onFilter(SearchResult searchResult, String searchTerm);
	}
