...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		int textPrimaryColor = a.getColor(TEXT_COLOR_PRIMARY, getResources()
		...
		mPaddingLeft = padding > 0 ? padding : a.getDimensionPixelSize(
				PADDING_LEFT_INDEX, 0);
		mPaddingRight = padding > 0 ? padding : a.getDimensionPixelSize(
				PADDING_RIGHT_INDEX, 0);
		a.recycle();
		...
		mTabTextSize = a.getDimensionPixelSize(
				R.styleable.PagerSlidingTabStrip_pstsTabTextSize, mTabTextSize);
		mTabTextColor = a
				.hasValue(R.styleable.PagerSlidingTabStrip_pstsTabTextColor) ? a
				.getColorStateList(R.styleable.PagerSlidingTabStrip_pstsTabTextColor)
				: null;
		...
		isTabTextAllCaps = a.getBoolean(
				...
				isTabTextAllCaps);
		int tabTextAlpha = a.getInt(
				R.styleable.PagerSlidingTabStrip_pstsTabTextAlpha,
				DEF_VALUE_TAB_TEXT_ALPHA);
		String fontFamily = a
				.getString(R.styleable.PagerSlidingTabStrip_pstsTabTextFontFamily);
		a.recycle();
		...
		mTabTextTypeface = Typeface.create(tabTextTypefaceName,
				mTabTextTypefaceStyle);
		...
		// or just wrap tabs
		mTabLayoutParams = isExpandTabs ? new LinearLayout.LayoutParams(0,
	...
	private void setTabsContainerParentViewPaddings() {
		int bottomMargin = mIndicatorHeight >= mUnderlineHeight ? mIndicatorHeight
				: mUnderlineHeight;
	...
	public void notifyDataSetChanged() {
		...
		for (int i = 0; i < mTabCount; i++) {
			if (isCustomTabs) {
				tabView = ((CustomTabProvider) mPager.getAdapter())
						.getCustomTabView(this, i);
			} else {
				tabView = LayoutInflater.from(getContext()).inflate(
						R.layout.psts_tab, this, false);
			}
	...
	private void addTab(final int position, CharSequence title, View tabView) {
		TextView textView = (TextView) tabView
				.findViewById(R.id.psts_tab_title);
		if (textView != null) {
			if (title != null)
		...
		tabView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (mPager.getCurrentItem() != position) {
					View tab = mTabsContainer.getChildAt(mPager
							.getCurrentItem());
					unSelect(tab);
	...
	private void updateTabStyles() {
		for (int i = 0; i < mTabCount; i++) {
			...
			v.setPadding(mTabPadding, v.getPaddingTop(), mTabPadding,
					v.getPaddingBottom());
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mTabsContainer.getChildCount() > 0) {
			mTabsContainer.getChildAt(0).getViewTreeObserver()
	...
	private OnGlobalLayoutListener firstTabGlobalLayoutListener = new OnGlobalLayoutListener() {
		...
		public void onGlobalLayout() {
			...
			if (isPaddingMiddle) {
				...
				mPaddingLeft = mPaddingRight = getWidth() / 2
						- mHalfWidthFirstTab;
			}

			setPadding(mPaddingLeft, getPaddingTop(), mPaddingRight,
					getPaddingBottom());
			if (mScrollOffset == 0)
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mUnderlineHeight > 0) {
			...
			canvas.drawRect(mPaddingLeft, height - mUnderlineHeight,
					mTabsContainer.getWidth() + mPaddingRight, height,
					mRectPaint);
		}
		...
		if (mIndicatorHeight > 0) {
			...
			Pair<Float, Float> lines = getIndicatorCoordinates();
			canvas.drawRect(lines.first + mPaddingLeft, height
					- mIndicatorHeight, lines.second + mPaddingLeft, height,
					mRectPaint);
		}
	}
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset,
			...
			int offset = mTabCount > 0 ? (int) (positionOffset * mTabsContainer
					.getChildAt(position).getWidth()) : 0;
		...
		public void onPageScrollStateChanged(int state) {
			...
			View currentTab = mTabsContainer
					.getChildAt(mPager.getCurrentItem());
			select(currentTab);
			// Half transparent for prev item
			if (mPager.getCurrentItem() - 1 >= 0) {
				View prevTab = mTabsContainer.getChildAt(mPager
						.getCurrentItem() - 1);
				unSelect(prevTab);
			}
			...
			if (mPager.getCurrentItem() + 1 <= mPager.getAdapter().getCount() - 1) {
				View nextTab = mTabsContainer.getChildAt(mPager
						.getCurrentItem() + 1);
				unSelect(nextTab);
			}
	...
	private void unSelect(View tab) {
		if (tab != null) {
			TextView tab_title = (TextView) tab
					.findViewById(R.id.psts_tab_title);
			if (tab_title != null) {
	...
	private ColorStateList createColorStateList(int color_state_pressed,
		...
		return new ColorStateList(new int[][] {
				...
				new int[] {} // default
				}, new int[] { color_state_pressed, color_state_selected,
						color_state_default });
	}
