 ...
 * 
 * Project: BottomSheet Created by LiaoKai(soarcn) on 2014/9/21.
 */
public class BottomSheet extends Dialog implements DialogInterface {
	...
	private void init(final Context context) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			mDialogView.setPadding(0, mStatusBarAvailable ? mStatusBarHeight
					: 0, 0, 0);
			...
			while (i.hasNext()) {
				...
				if (item.divider)
					...
					throw new IllegalArgumentException(
							"You should set icon for each items in grid style");
				}
			}
		}
					...
					if (convertView == null) {
						...
						if (builder.grid)
							convertView = inflater.inflate(
									R.layout.bs_grid_entry, null);
						else
							convertView = inflater.inflate(
									R.layout.bs_list_entry, null);
						holder = new ViewHolder();
						holder.title = (TextView) convertView
								.findViewById(R.id.bs_list_title);
						holder.image = (ImageView) convertView
								.findViewById(R.id.bs_list_image);
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (builder.listener != null) {
					builder.listener.onClick(BottomSheet.this,
							((MenuItem) adapter.getItem(position)).id);
				}
				dismiss();
			}
		});
	...
	private static class MenuItem {
		...
		public String toString() {
			return "MenuItem{" + "id=" + id + ", text=" + text + ", icon="
	...
	public static class Builder {
					...
					if (eventType == XmlPullParser.START_TAG) {
						...
						if (elemName.equals("item")) {
							...
							if (!TextUtils.isEmpty(iconId))
								item.icon = context.getResources()
										.getDrawable(
		...
		public Builder sheet(int id, @DrawableRes int iconRes,
			...
			item(new MenuItem(id, context.getText(textRes), context
		...
		/**
		 * Add a divider in to the list Be aware divider would not be shown in
		 * grid mode
		 * 
		 * @return This Builder object to allow for chaining of calls to set
		 *         methods
		 ...
		 * If more actions need to be shown, a "more" action will be display in
		 * the last position.
		 * 
		 * @param limitRes
		 * @return
		...
		/**
		 * Deprecated please use build() Create a BottomSheet but not show it
		 * 
		 * @return This Builder object to allow for chaining of calls to set
		 *         methods
		 */
